class elevator:
    ...
    #      if self.time <= call.time:
    #         return call.time + timeOfWay
    #      if self.time > call.time:
    #          # both of them are going up and the call is in the way of the elevator
    #          if call.ud & (self.dest - self.src) > 0 & call.source <= self.src:
    #              if call.dest <= self.dest: # the call dest is into the way
    #                  timeOfWay = timeOfWay + 2 * self.stopTime + 2 * self.closeTime + 2 * self.openTime + 2 * self.startTime
    #                  return timeOfWay
    #              else: # the call dest is after the way
    #                 timeOfWay = timeOfWay + 2 * self.stopTime + 2 * self.closeTime + 2 * self.openTime + 2 * self.startTime + abs(call.dest - self.dest) / self.speed
    #                 return timeOfWay
    #          # both of them are going down and the call is in the way of the elevator
    #          if call.ud & (self.dest - self.src) < 0 & call.source >= self.src:
    #              if  self.dest < call.dest: # the call dest is into the way
    #                  timeOfWay = timeOfWay + 2 * self.stopTime + 2 * self.closeTime + 2 * self.openTime + 2 * self.startTime
    #                  return timeOfWay
    #              else: # the call dest is after the way
    #                 timeOfWay = timeOfWay + 2 * self.stopTime + 2 * self.closeTime + 2 * self.openTime + 2 * self.startTime + abs(call.dest - self.dest) / self.speed
